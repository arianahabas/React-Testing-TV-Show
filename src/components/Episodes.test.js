...
test('re-renders episodes correctly on click of chosen season', ()=>{

    //Arrange (render the component and setup mock data)
    const { rerender, getAllByTestId, queryAllByTestId } = render(<Episodes episodes={[]} />)
    //check that there are no episodes yet when component first mounts
    ...
    // (simulate when the user clicks on seasons button)
    ...
    // 3 episodes should render to the page
