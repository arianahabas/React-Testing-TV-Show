...
import App from './App'
import { render,  getByText, waitFor, screen, findAllByTestId } from '@testing-library/react'
...
test('episodes render upon API Call', async () => {
    //mock the returned value of fetchShow
    //setup work for when fetchShow will eventually be called
    mockFetchShow.mockResolvedValueOnce(episodesData)
    //render App
    render(<App />);
    
    expect(await screen.findByText(/a love letter/i)).toBeInTheDocument()

    //query for and click the dropdown button
